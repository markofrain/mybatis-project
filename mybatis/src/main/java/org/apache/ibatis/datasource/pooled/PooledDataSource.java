...
public class PooledDataSource implements DataSource {
  ...
  /**
   * PoolState管理连接池的状态并记录统计信息
   */
  private final PoolState state = new PoolState(this);
  /**
   * 记录UnpooledDataSource对象，用于生成真实的数据库连接对象，直接由该对象创建连接。
   */
  ...
  /**
   * 当超过poolPingConnectionsNotUsedFor毫秒未使用时，会发送一次测试sql语句，监测连接是否正常
   */
  protected int poolPingConnectionsNotUsedFor;
  /**
   * 根据数据库url，用户名，密码生成的hash值，用于标志当前的连接池，在构造函数中初始化
   */
