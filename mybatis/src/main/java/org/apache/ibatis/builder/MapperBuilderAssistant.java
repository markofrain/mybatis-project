...
public class MapperBuilderAssistant extends BaseBuilder {
  ...
  public Cache useCacheRef(String namespace) {
    ...
    try {
      ...
      // 获取namespace对应的Cache对象,此处确保了cache-ref的namespace必须存在了Cache对象
      Cache cache = configuration.getCache(namespace);
      if (cache == null) {
        throw new IncompleteElementException("No cache for namespace '" + namespace + "' could be found.");
      }
      // 记录当前命名空间的缓存对象为引用命名空间的缓存对象
      currentCache = cache;
      // 标识已成功解析Cache引用
  ...
  /**
   * 创建一个新的Cache对象并加入到configuration中
   * @param typeClass
   * @param evictionClass
   * @param flushInterval
   * @param size
  ...
  public Cache useNewCache(Class<? extends Cache> typeClass,
    ...
    Cache cache = new CacheBuilder(currentNamespace)
        ...
        .properties(props)
        .build();
    // 将Cache对象添加到caches集合中保存,将cache的id(namespace)作为key，cache作为本身value
  ...
  /**
   * 添加resultMap到configuration.resultMaps
   * @param id
   * @param type
   * @param extend
   * @param discriminator
  ...
  public ResultMap addResultMap(
      ...
      Boolean autoMapping) {
    // ResultMap的完整id是"namespace.id"格式
    id = applyCurrentNamespace(id, false);
    ...
    if (extend != null) {
      ...
      // 如果当前resultMap节点定义了constructor节点，则不需要使用父ResultMap中记录中的相应constructor节点
      ...
      // 删除父ResultMap中的覆盖constructor节点ResultMapping
      if (declaresConstructor) {
        extendedResultMappings.removeIf(resultMapping -> resultMapping.getFlags().contains(ResultFlag.CONSTRUCTOR));
      }
      //添加被继承下来的ResultMapping命令
